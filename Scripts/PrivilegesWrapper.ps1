...
/*
Removed LINQ to be able to use Add-Type:
using System.Linq;
*/
using System.Text;
...
//
// Got from: https://social.technet.microsoft.com/Forums/office/en-US/d8d1f997-84d1-4e7b-9c66-c609f780f214/query-account-privileges-for-context-running-script
...
namespace LSAController {
    ...
    public class LocalSecurityAuthorityController {
        ...
        public IList<string> GetRights(string accountName) {
            ...
            int sidSize = 0;
            StringBuilder domainName = new StringBuilder();S
            int nameSize = 0;
            int accountType = 0;
            ...
            if (!LookupAccountName(string.Empty, accountName, sid, ref sidSize, domainName, ref nameSize, ref accountType)) {
                ...
                if (winErrorCode != 0) {
                    ...
                    //Int32 ptr = userRightsPtr.ToInt32(); // changed
        ...
        /*
			Function removed as to avoid LINQ
...
Function Get-UserPrivileges($AccountName) {
	return [LSAController.LSAWrapper]::GetRights($AccountName)
}

Function Set-UserPrivileges($AccountName, $PrivilegeName) {
    return [LSAController.LSAWrapper]::SetRight($AccountName, $PrivilegeName)
}